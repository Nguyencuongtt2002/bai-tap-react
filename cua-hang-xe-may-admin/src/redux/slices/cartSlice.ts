...
// Định nghĩa kiểu dữ liệu cho trạng thái của slice cart
interface CartState {
    items: Product[];
...
export const cartSlice = createSlice({
    ...
    reducers: {
        ...
        xoa1sp: (state, action: PayloadAction<number>) => {
        ...
        tangSoLuong: (state, action: PayloadAction<number>) => {
            const MaSanPham = action.payload;
        ...
        giamSoLuong: (state, action: PayloadAction<number>) => {
            const MaSanPham = action.payload;
